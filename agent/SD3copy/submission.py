...
class Actor(nn.Module):
    def __init__(self, obs_dim, act_dim, num_agents, args, output_activation='softmax'):
        ...
        sizes_post = [HIDDEN_SIZE, HIDDEN_SIZE, HIDDEN_SIZE, act_dim]
...
class RLAgent(object):
    def __init__(self, obs_dim, act_dim, num_agent):
        ...
        self.actor1 = Actor(obs_dim, act_dim, num_agent, self.output_activation).to(self.device)
        self.actor2 = Actor(obs_dim, act_dim, num_agent, self.output_activation).to(self.device)
        self.critic1 = Critic(obs_dim, act_dim, num_agent).to(self.device)
        self.critic2 = Critic(obs_dim, act_dim, num_agent).to(self.device)
    ...
    def load_model(self, actor1, actor2, critic1, critic2):
        self.actor1.load_state_dict(torch.load(actor1, map_location='cpu'))
        self.actor2.load_state_dict(torch.load(actor2, map_location='cpu'))
        self.critic1.load_state_dict(torch.load(critic1, map_location='cpu'))
        self.critic2.load_state_dict(torch.load(critic2, map_location='cpu'))
...
epinum = 2000
# actor1_path = '/actor1_' + str(epinum) + '.pth'
actor1_net = os.path.dirname(os.path.abspath(__file__)) + '/actor1_' + str(epinum) + '.pth'
actor2_net = os.path.dirname(os.path.abspath(__file__)) + '/actor2_' + str(epinum) + '.pth'
critic1_net = os.path.dirname(os.path.abspath(__file__)) + '/critic1_' + str(epinum) + '.pth'
critic2_net = os.path.dirname(os.path.abspath(__file__)) + '/critic2_' + str(epinum) + '.pth'
# actor1_net = os.path.dirname(os.path.abspath(__file__)) + '/actor1_1000.pth'
# actor2_net = os.path.dirname(os.path.abspath(__file__)) + '/actor2_1000.pth'
# critic1_net = os.path.dirname(os.path.abspath(__file__)) + '/critic1_1000.pth'
# critic2_net = os.path.dirname(os.path.abspath(__file__)) + '/critic2_1000.pth'
agent.load_model(actor1_net,actor2_net,critic1_net,critic2_net)
