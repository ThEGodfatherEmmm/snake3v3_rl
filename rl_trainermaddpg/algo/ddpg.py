...
class DDPG:
    ...
    def __init__(self, obs_dim, act_dim, num_agent, args):
        ...
        self.actor1 = Actor(obs_dim, act_dim, self.num_agent, args, self.output_activation).to(self.device)
        self.actor2 = Actor(obs_dim, act_dim, self.num_agent, args, self.output_activation).to(self.device)
        self.actor3 = Actor(obs_dim, act_dim, self.num_agent, args, self.output_activation).to(self.device)
        ...
        self.actor3_optimizer = torch.optim.Adam(self.actor3.parameters(), lr=self.a_lr)
        self.critic_optimizer = torch.optim.Adam(self.critic.parameters(), lr=self.c_lr)

        # Initialise target network and critic network with ξ' ← ξ and θ' ← θ
        self.actor1_target = Actor(obs_dim, act_dim, self.num_agent, args, self.output_activation).to(self.device)
        self.actor2_target = Actor(obs_dim, act_dim, self.num_agent, args, self.output_activation).to(self.device)
        self.actor3_target = Actor(obs_dim, act_dim, self.num_agent, args, self.output_activation).to(self.device)
        ...
        hard_update(self.actor3, self.actor3_target)
    ...
    def choose_action(self, obs, evaluation=False):
        ...
        if p > self.eps or evaluation:
            ...
            action.append(self.actor1(obs[0][0]).cpu().detach().numpy()[0])
            action.append(self.actor2(obs[0][1]).cpu().detach().numpy()[0])
            action.append(self.actor3(obs[0][2]).cpu().detach().numpy()[0])
        else:
    ...
    def update(self):
        ...
        with torch.no_grad():
            ...
            target_next_actions = []
            target_next_actions.append(self.actor1_target(next_state_batch[0][0]))
