...
from algo.SD3 import SD3
from TD3.submission import agent as TDagent
from common import *
...
def main(args):
    ...
    model = SD3(obs_dim, act_dim, ctrl_agent_num, args)
    ...
    while episode < args.max_episodes:
        ...
        while True:
            ...
            TDlogits = TDagent.choose_action(obs)            
            actions = logits_TD(logits, TDlogits)
            ...
            if args.episode_length <= step or (True in done):
                ...
                print(f'[Episode {episode:05d}] total_reward: {np.sum(episode_reward[0:3]):}')
                ...
                if model.c1_loss and model.a1_loss:
                    writer.add_scalars(loss_tag, global_step=episode,
                                       tag_scalar_dict={'actor1': model.a1_loss, 'critic1': model.c1_loss, 
                                       'actor2': model.a2_loss, 'critic2': model.c2_loss})

                if model.c1_loss and model.a1_loss:
                    print(f'\t\t\t\ta1_loss {model.a1_loss:.3f} c1_loss {model.c1_loss:.3f}')
                    print(f'\t\t\t\ta2_loss {model.a2_loss:.3f} c2_loss {model.c2_loss:.3f}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--algo', default="SD3", type=str, help="bicnet/ddpg")
    ...
    parser.add_argument('--c_lr', default=0.0002, type=float)
    ...
    parser.add_argument("--load_model_run", default=0, type=int)
